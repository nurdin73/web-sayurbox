              ...
              <tbody>
                {data.map((v,k)=>(
                  <tr key={v.id}>
                    ...
                    <td>
                     <button style={{border:"none"}} onClick={()=>this.setState({paymentDetail: !this.state.paymentDetail})}>
                      <img alt="product" src={v.foto} style={{width: 60, height: 60}} />
                     </button>
                    </td>
                    <td>
                    <div class="btn-group" role="group" aria-label="Basic example">
                        <Button color={transactionStatus === "pending" ? "primary" : "warning"}>
                          {transactionStatus === "pending" ? <i className="fa fa-forward">&nbsp;Proses</i> : <i className="fa fa-check">Selesaikan</i>}
                        </Button>
                       <Button color="secondary" onClick={()=>this.setState({transactionDetail: !transactionDetail})}>
                        <i className="fa fa-info-circle">&nbsp;Detail</i>
                       </Button>
                        <button className="btn btn-sm btn-danger"><i className="fa fa-close "></i> Cancel</button> 
                      </div>
                    </td>
                  </tr>
                ))}
